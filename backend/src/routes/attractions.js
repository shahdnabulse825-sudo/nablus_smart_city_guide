...
// نخزّن القوائم (categories) كنص واحد مفصول بفواصل (SQLite ما بيدعم مصفوفات)
...
router.get('/', async (req, res) => {
  const items = await prisma.attraction.findMany({ orderBy: [{ isFeatured: 'desc' }, { rating: 'desc' }] });
  res.json(items);
});

router.get('/:id', async (req, res) => {
  const item = await prisma.attraction.findUnique({ where: { id: req.params.id } });
  if (!item) return res.status(404).json({ error: 'المعلم غير موجود' });
  res.json(item);
});

router.post('/', requireAuth, requireAdmin, upload.single('image'), async (req, res) => {
  const b = req.body;
  const item = await prisma.attraction.create({
    data: {
      nameAr: b.nameAr || '',
      nameEn: b.nameEn || '',
      categories: listField(b.categories),
      ...
      aboutAr: b.aboutAr || '',
      aboutEn: b.aboutEn || '',
      visitHoursAr: b.visitHoursAr || '',
      visitHoursEn: b.visitHoursEn || '',
      entryFeeAr: b.entryFeeAr || '',
      entryFeeEn: b.entryFeeEn || '',
      imageUrl: req.file ? `/uploads/${req.file.filename}` : null,
      iconCodePoint: numField(b.iconCodePoint, 0xe84f),
      colorValue: numField(b.colorValue, 0xc9a227) & 0xffffff,
...
router.put('/:id', requireAuth, requireAdmin, upload.single('image'), async (req, res) => {
  ...
  const item = await prisma.attraction.update({
    ...
    data: {
      ...
      categories: b.categories !== undefined ? listField(b.categories) : existing.categories,
      ...
      visitHoursAr: b.visitHoursAr ?? existing.visitHoursAr,
      visitHoursEn: b.visitHoursEn ?? existing.visitHoursEn,
      entryFeeAr: b.entryFeeAr ?? existing.entryFeeAr,
      entryFeeEn: b.entryFeeEn ?? existing.entryFeeEn,
